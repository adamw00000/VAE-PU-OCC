...
class VaePuTrainer():
    ...
    def train(self, vae_pu_data):
        ...
        trained_vae_pu_exists = os.path.exists(os.path.join(
            self.config['directory'], 'model_pre_occ.pt'))
        if self.config['use_old_models'] and not self.use_original_paper_code and trained_vae_pu_exists:
            ...
            for epoch in range(self.config['num_epoch']):
                if not self.use_original_paper_code:
                    self.log = open(os.path.join(
                        self.config['directory'], 'log.txt'), 'a')
                    self.log2 = open(os.path.join(
                        self.config['directory'], 'log_PN.txt'), 'a')
                ...
                if epoch < self.config['num_epoch_step1']:
                    # train autoencoder only, no label loss
                    if (epoch + 1) % 100 == 0:
                        self._check_discriminator_and_classifier(
                            epoch, x_pl, x_u)
                    ...
                    if (epoch + 1) % 100 == 0:
                        self._check_discriminator_and_classifier(
                            epoch, x_pl, x_u)
                    self._calculate_target_classifier_metrics(
                        epoch, epoch_losses)
    ...
    def _save_final_vae_pu_metric_values(self, metric_values):
        if not self.use_original_paper_code:
            with open(os.path.join(self.config['directory'], 'metric_values.json'), 'w') as f:
                json.dump(metric_values, f)
            with open(os.path.join(self.config['directory'], 'settings.json'), 'w') as f:
                json.dump({
                    'Label frequency': self.config['label_frequency'].item(),
                    'Pi': self.config['pi_p'].item(),
                    'True storey pi': (self.config["pi_pu"] / self.config["pi_u"]).item()
                }, f)

            self._plotLoss(self.targetClassifierLosses, os.path.join(
                self.config['directory'], 'loss_PN.png'))
            self._plotLoss(self.valAccuracies, os.path.join(
                self.config['directory'], 'val_accuracy.png'))
            ...
            if self.config['train_occ']:
                log2.write('final test pre-occ: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1-score: {3:.4f}'.format(acc, precision, recall, f1_score) + '\n')
                print('final test pre-occ: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1-score: {3:.4f}'.format(acc, precision, recall, f1_score))
            else:
            ...
            with open(os.path.join(self.config['directory'], 'metric_values_orig.json'), 'w') as f:
                json.dump(metric_values, f)
    ...
    def _calculate_target_classifier_metrics(self, epoch, losses):
        ...
        if not self.use_original_paper_code:
            self.log2.write('epoch: {}, loss: {}'.format(
                epoch + 1, targetLoss) + '\n')
        ...
        if not self.use_original_paper_code:
            self.log2.write('...val: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1: {3:.4f}'.format(
                val_acc, val_pr, val_re, val_f1) + '\n')
        print('...val: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1: {3:.4f}'.format(
            val_acc, val_pr, val_re, val_f1))
    ...
    def _calculate_VAE_metrics(self):
        val_acc, val_pr, val_re, val_f1 = self.model.accuracy(
            self.DL_val, use_vae=True)
        if not self.use_original_paper_code:
            self.log.write('...(VAE) val: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1: {3:.4f}'.format(
                val_acc, val_pr, val_re, val_f1) + '\n')
        print('...(VAE) val: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1: {3:.4f}'.format(
            val_acc, val_pr, val_re, val_f1))

    def _print_autoencoder_log(self, epoch, losses):
        elbo, advGenLoss, discLoss, labelLoss = np.mean(losses['ELBO']), np.mean(
            losses['Adversarial generation']), np.mean(losses['Discriminator']), np.mean(losses['Label'])
        ...
        print(f'Exp: {self.num_exp} / c = {self.config["base_label_frequency"]:.2f} / Epoch: {epoch + 1:4} |||| ELBO loss: {elbo:.4f}, AdvGen loss: {advGenLoss:.4f}, Disc loss: {discLoss:.4f}, Label loss {labelLoss:.4f}')
    ...
    def _check_discriminator_and_classifier(self, epoch, x_pl, x_u):
        ...
        if not self.use_original_paper_code:
            self.log.write('d_x_pu: {}, d_x_u: {}\n'.format(
                torch.mean(d_x_pu), torch.mean(d_x_u)))
            self.log.write('d_x_pu2: {}, d_x_pl2: {}\n'.format(
    ...
    def _pretrain_autoencoder(self):
        ...
        for epoch in range(self.config['num_epoch_pre']):
            ...
            end_time = time.time()
            print('[PRE-TRAIN] Remaining time: {} sec'.format(
        ...
        self._plotLoss(self.preLoss1, os.path.join(
            self.config['directory'], 'loss_pretrain.png'))
        if self.config['bool_pn_pre']:
            self._plotLoss(self.preLoss2, os.path.join(
                self.config['directory'], 'loss_pretrain_pn.png'))
    ...
    def _prepare_log_files(self):
        ...
        if not self.use_original_paper_code:
            self.log = open(os.path.join(
                self.config['directory'], 'log.txt'), 'w')
            ...
            self.log2 = open(os.path.join(
                self.config['directory'], 'log_PN.txt'), 'w')
    ...
    def _prepare_dataloaders(self, vae_pu_data):
        self.x_pl_full, self.y_pl_full, self.x_u_full, self.y_u_full, self.x_val, self.y_val, self.x_test, self.y_test = vae_pu_data

        if 'MNIST' in self.config['data']:
            self.x_pl_full = (self.x_pl_full + 1.) / 2.
            self.x_u_full = (self.x_u_full + 1.) / 2.
            self.x_val = (self.x_val + 1.) / 2.
            self.x_test = (self.x_test + 1.) / 2.

        self.DL_pl = InfiniteDataLoader(TensorDataset(
            self.x_pl_full), batch_size=self.config['batch_size_l'], shuffle=True)
        self.DL_u = DataLoader(TensorDataset(
            self.x_u_full), batch_size=self.config['batch_size_u'], shuffle=True)
        self.DL_u_full = DataLoader(TensorDataset(
            self.x_u_full, self.y_u_full), batch_size=self.config['batch_size_u'], shuffle=True)
        self.DL_val = DataLoader(TensorDataset(
            self.x_val, self.y_val), batch_size=self.config['batch_size_val'], shuffle=True)
        self.DL_test = DataLoader(TensorDataset(
            self.x_test, self.y_test), batch_size=self.config['batch_size_test'], shuffle=True)
    ...
    def _prepare_model(self):
        ...
        opt_en = Adam(model_en.parameters(),
                      lr=self.config['lr_pu'], eps=1e-07)
        opt_de = Adam(model_de.parameters(),
                      lr=self.config['lr_pu'], eps=1e-07)
        opt_disc = Adam(model_disc.parameters(), lr=self.config['lr_disc'], betas=(
            self.config['beta1'], self.config['beta2']), eps=1e-07)
        opt_cl = Adam(model_cl.parameters(),
                      lr=self.config['lr_pu'], weight_decay=1e-5, eps=1e-07)
        opt_pn = Adam(model_pn.parameters(),
                      lr=self.config['lr_pn'], weight_decay=1e-5, eps=1e-07)

        self.model = myPU(self.config, model_en, model_de, model_disc,
                          model_cl, model_pn, opt_en, opt_de, opt_disc, opt_cl, opt_pn)

    def _load_trained_vae_pu(self):
        model = torch.load(os.path.join(
            self.config['directory'], 'model_pre_occ.pt'))
        with open(os.path.join(self.config['directory'], 'metric_values.json'), 'r') as f:
            metric_values = json.load(f)
        self.acc_pre_occ, self.precision_pre_occ, self.recall_pre_occ, self.f1_pre_occ = \
            metric_values['Accuracy'], metric_values['Precision'], metric_values['Recall'], metric_values['F1 score']
        ...
        print('final test pre-occ: acc: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, f1-score: {3:.4f}'.format(
            self.acc_pre_occ, self.precision_pre_occ, self.recall_pre_occ, self.f1_pre_occ))
        return model
