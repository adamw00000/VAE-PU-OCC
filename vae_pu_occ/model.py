...
class VAEencoder(nn.Module):
    def __init__(self, config):
        ...
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_input']] + self.config['n_hidden_vae_e'][:-1], self.config['n_hidden_vae_e'])):
            self.vae_en_y.add_module(f"linear_{i}", nn.Linear(in_neurons, out_neurons))
            self.vae_en_y.add_module(f"batch_norm_{i}", nn.BatchNorm1d(out_neurons, momentum=0.99, eps=0.001))
            self.vae_en_y.add_module(f"leaky_relu_{i}", nn.LeakyReLU(0.2))

        self.vae_en_o = nn.Sequential()
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_input'] + self.config['n_o']] + self.config['n_hidden_vae_e'][:-1], self.config['n_hidden_vae_e'])):
            self.vae_en_o.add_module(f"linear_{i}", nn.Linear(in_neurons, out_neurons))
            self.vae_en_o.add_module(f"batch_norm_{i}", nn.BatchNorm1d(out_neurons, momentum=0.99, eps=0.001))
            self.vae_en_o.add_module(f"leaky_relu_{i}", nn.LeakyReLU(0.2))
        
        self.vae_en_y_mu = nn.Sequential()
        self.vae_en_y_mu.add_module(f"linear_y_mu", nn.Linear(self.config['n_hidden_vae_e'][-1], self.config['n_h_y']))

        self.vae_en_y_lss = nn.Sequential()
        self.vae_en_y_lss.add_module(f"linear_y_lss", nn.Linear(self.config['n_hidden_vae_e'][-1], self.config['n_h_y']))

        self.vae_en_o_mu = nn.Sequential()
        self.vae_en_o_mu.add_module(f"linear_o_mu", nn.Linear(self.config['n_hidden_vae_e'][-1], self.config['n_h_o']))

        self.vae_en_o_lss = nn.Sequential()
        self.vae_en_o_lss.add_module(f"linear_o_lss", nn.Linear(self.config['n_hidden_vae_e'][-1], self.config['n_h_o']))
...
class VAEdecoder(nn.Module):
    def __init__(self, config):
        ...
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_h_y'] + self.config['n_h_o']] + self.config['n_hidden_vae_d'][:-1], self.config['n_hidden_vae_d'])):
            self.vae_de.add_module(f"linear_{i}", nn.Linear(in_neurons, out_neurons))
            self.vae_de.add_module(f"batch_norm_{i}", nn.BatchNorm1d(out_neurons, momentum=0.99, eps=0.001))
            self.vae_de.add_module(f"leaky_relu_{i}", nn.LeakyReLU(0.2))
        self.vae_de.add_module(f"linear_out", nn.Linear(self.config['n_hidden_vae_d'][-1], self.config['n_input']))
...
class discriminator(nn.Module):
    def __init__(self, config):
        ...
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_input']] + self.config['n_hidden_disc'][:-1], self.config['n_hidden_disc'])):
        ...
        last_shape = self.config['n_hidden_disc'][-1] if len(self.config['n_hidden_disc']) > 0 else self.config['n_input']
...
class classifier_o(nn.Module):
    def __init__(self, config):
        ...
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_h_o']] + self.config['n_hidden_cl'][:-1], self.config['n_hidden_cl'])):
            self.classification.add_module(f"linear_{i}", nn.Linear(in_neurons, out_neurons))
            self.classification.add_module(f"batch_norm_{i}", nn.BatchNorm1d(out_neurons, momentum=0.99, eps=0.001))
            self.classification.add_module(f"leaky_relu_{i}", nn.LeakyReLU(0.2))
        
        last_shape = self.config['n_hidden_cl'][-1] if len(self.config['n_hidden_cl']) > 0 else self.config['n_h_o']
...
class classifier_pn(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.classification = nn.Sequential()
        for i, (in_neurons, out_neurons) in enumerate(zip([self.config['n_input']] + self.config['n_hidden_pn'][:-1], self.config['n_hidden_pn'])):
            self.classification.add_module(f"linear_{i}", nn.Linear(in_neurons, out_neurons))
            self.classification.add_module(f"batch_norm_{i}", nn.BatchNorm1d(out_neurons, momentum=0.99, eps=0.001))
            self.classification.add_module(f"leaky_relu_{i}", nn.LeakyReLU(0.2))
        
        last_shape = self.config['n_hidden_pn'][-1] if len(self.config['n_hidden_pn']) > 0 else self.config['n_input']
        self.classification.add_module(f"linear_out", nn.Linear(last_shape, 1))
...
class myPU():
    def __init__(self, config, model_en, model_de, model_disc, model_cl, model_pn, opt_en, opt_de, opt_disc, opt_cl, opt_pn, \
            opt_param=None, use_original_paper_code=False):
        self.config = config
    ...
    def reparameterization(self, mu, log_sig_sq):
        ...
        return mu + torch.exp(log_sig_sq / 2.) * eps

    def concat_data(self, x_pl, x_u):
        x = torch.cat([x_pl, x_u], axis=0)

        o_pl = torch.cat([torch.ones([x_pl.shape[0], 1]).to(self.config['device']), torch.zeros([x_pl.shape[0], 1]).to(self.config['device'])], axis=1)
        o_u = torch.cat([torch.zeros([x_u.shape[0], 1]).to(self.config['device']), torch.ones([x_u.shape[0], 1]).to(self.config['device'])], axis=1)
    ...
    def pretrain(self, x_pl, x_u):
        ...
        if 'MNIST' in self.config['data']:
            ...
            loss = torch.mean(n_features * F.binary_cross_entropy_with_logits(input=recon_x, target=x))
    ...
    def findPrior(self, x_tr_l, x_tr_u):
        ...
        o_pl = torch.cat([torch.ones([x_tr_l.shape[0], 1]).to(self.config['device']), torch.zeros([x_tr_l.shape[0], 1]).to(self.config['device'])], axis=1)
        o_u = torch.cat([torch.zeros([x_tr_u.shape[0], 1]).to(self.config['device']), torch.ones([x_tr_u.shape[0], 1]).to(self.config['device'])], axis=1)
        ...
        self.mu = torch.tensor(np.asarray(gmm.means_, dtype=np.float32)).to(self.config['device'])
        self.var = torch.tensor(np.asarray(gmm.covariances_, dtype=np.float32)).to(self.config['device'])

        c0 = torch.mean(-0.5 * torch.true_divide(torch.square(h_y_l - self.mu[0]), self.var[0]) - 0.5 * torch.log(self.var[0] + 1e-9), axis=1)
        c1 = torch.mean(-0.5 * torch.true_divide(torch.square(h_y_l - self.mu[1]), self.var[1]) - 0.5 * torch.log(self.var[1] + 1e-9), axis=1)

        num0 = torch.sum(torch.greater(c0 - c1, 0.).int())
        frac0 = num0 / x_tr_l.shape[0]
        if frac0 > 0.5:
            self.p = torch.tensor(gmm.weights_[0]).to(self.config['device'])
            self.mu[0], self.mu[1] = torch.tensor(gmm.means_[1]).to(self.config['device']), torch.tensor(gmm.means_[0]).to(self.config['device'])
            self.var[0], self.var[1] = torch.tensor(gmm.covariances_[1]).to(self.config['device']), torch.tensor(gmm.covariances_[0]).to(self.config['device'])
        self.mu = torch.tensor(np.asarray(self.mu.cpu(), dtype=np.float32)).to(self.config['device'])
        self.var = torch.tensor(np.asarray(self.var.cpu(), dtype=np.float32)).to(self.config['device'])
    ...
    def train_step_vae(self, x_pl, x_u, epoch):
        ...
        c0 = -0.5 * torch.true_divide(torch.square(h_y - self.mu[0]), self.var[0]) - 0.5 * torch.log(torch.clamp_min(self.var[0], 1e-9)) + torch.log(torch.clamp_min(1 - p, 1e-9))
        c1 = -0.5 * torch.true_divide(torch.square(h_y - self.mu[1]), self.var[1]) - 0.5 * torch.log(torch.clamp_min(self.var[1], 1e-9)) + torch.log(torch.clamp_min(p, 1e-9))
        ...
        gan_loss = alpha_gen * torch.mean(F.binary_cross_entropy_with_logits(input=d_x_pu, target=label))
        ...
        if epoch <= self.config['num_epoch_step1']:
            ...
            gan_loss2 = alpha_gen2 * torch.mean(self.sigmoid_loss(d_x_pu2, torch.ones_like(d_x_pu2)))

        loss1_0 = - torch.sum(0.5 * (
            torch.log(torch.clamp_min(self.var[0], 1e-9)) + \
            torch.true_divide(torch.exp(h_y_log_sig_sq) + \
            torch.square(h_y_mu - self.mu[0]), self.var[0])), axis=1, keepdim=True)
        loss1_1 = - torch.sum(0.5 * (
            torch.log(torch.clamp_min(self.var[1], 1e-9)) + \
            torch.true_divide(torch.exp(h_y_log_sig_sq) + \
            torch.square(h_y_mu - self.mu[1]), self.var[1])), axis=1, keepdim=True)

        loss1 = torch.mean(torch.multiply(1 - c, loss1_0) + torch.multiply(c, loss1_1))
        loss2 = - torch.mean(torch.sum(0.5 * (torch.exp(h_o_log_sig_sq) + torch.square(h_o_mu)), axis=1))

        if 'MNIST' in self.config['data']:
            recon_x = self.model_de.decode(h_y, h_o)
            loss3 = - torch.mean(n_features * F.binary_cross_entropy_with_logits(input=recon_x, target=x))
        elif 'CIFAR' in self.config['data']:
            recon_x = self.model_de.decode(h_y, h_o)
            loss3 = self.config['alpha_test'] * - torch.mean(torch.sum(0.5 * torch.square(x - recon_x), axis=1))
        elif 'STL' in self.config['data']:
            recon_x = self.model_de.decode(h_y, h_o)
            loss3 = self.config['alpha_test'] * - torch.mean(torch.sum(0.5 * torch.square(x - recon_x), axis=1))
        else:
        ...
        loss6 = torch.mean(- c * torch.log((c / torch.clamp_min(p, 1e-9)) + 1e-9) - (1 - c) * torch.log(((1 - c) / torch.clamp_min(1-p, 1e-9)) + 1e-9))
        ...
        return vade_loss.detach().cpu().numpy(), gan_loss.detach().cpu().numpy(), gan_loss2.detach().cpu().numpy()

    def encode_y(self, x):
        x, _ = self.concat_data(x, torch.empty((0, x.shape[1])).to(self.config['device']))
    ...
    def get_pu_from_clustering(self, x_pl, x_u, storey_pi, clustering_metric):
        ...
        if clustering_metric == 'mean':
            ...
            sorted_distances = sorted_distances[:, :round(q * sorted_distances.shape[1])]
    ...
    def get_pu_from_clustering_batched(self, x_pl, x_u_batch, n_pu_samples, clustering_metric, \
            batch_start_idx=0, best_distances=None, best_xus=None, best_pu_indices=None):
        h_y_pl = self.encode_y(x_pl)
        ...
        best_pu_indices = torch.cat([pu_indices_batch + batch_start_idx, best_pu_indices[pu_indices_best]])

        del distance, distance_metric, x_u_sorted_idx, pu_indices, pu_indices_batch, pu_indices_best
    ...
    def vae_predict(self, x):
        ...
        return torch.where(proba > .5, 1, -1)

    def generate(self, x_pl, x_u, mode='near_o'):
        if mode == 'near_o':
            # nearest h_o
            o_pl = torch.tensor(np.concatenate([np.ones([x_pl.shape[0], 1]), np.zeros([x_pl.shape[0], 1])], axis=1)).float().to(self.config['device'])
            o_u = torch.tensor(np.concatenate([np.zeros([x_u.shape[0], 1]), np.ones([x_u.shape[0], 1])], axis=1)).float().to(self.config['device'])
    ...
    def train_step_disc(self, x_pl, x_u):
        ...
        disc_loss1 = alpha_disc * (torch.mean(F.binary_cross_entropy_with_logits(input=d_x_pu, target=label_pu)) + torch.mean(F.binary_cross_entropy_with_logits(input=d_x_u, target=label_u)))
    ...
    def train_step_pn_pre(self, x_pl, x_u):
        ...
        pu2_loss = torch.mean(- pi_p * self.sigmoid_loss(pn_x_pl, -torch.ones_like(pn_x_pl)))
    ...
    def compare(self, fname, x_pl, x_u, n=1):
        ...
        if 'MNIST' in self.config['data']:
            for i in range(1, n + 1):
                ...
                plt.imshow(x_pl[i - 1].detach().cpu().numpy().reshape(28, 28), vmin=0, vmax=1, cmap="gray")
                plt.title("real")

                plt.subplot(n, 5, (i - 1) * 5 + 2)
                plt.imshow(recon_x_pl[i - 1].detach().cpu().numpy().reshape(28, 28), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pl")

                plt.subplot(n, 5, (i - 1) * 5 + 3)
                plt.imshow(lst_x_pu[0][i - 1].detach().cpu().numpy().reshape(28, 28), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu1")

                plt.subplot(n, 5, (i - 1) * 5 + 4)
                plt.imshow(lst_x_pu[1][i - 1].detach().cpu().numpy().reshape(28, 28), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu2")

                plt.subplot(n, 5, (i - 1) * 5 + 5)
                plt.imshow(lst_x_pu[2][i - 1].detach().cpu().numpy().reshape(28, 28), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu3")
        else:
            for i in range(1, n + 1):
                plt.subplot(n, 5, (i - 1) * 5 + 1)
                plt.imshow(x_pl[i - 1].detach().cpu().numpy(), vmin=0, vmax=1, cmap="gray")
                plt.title("real")

                plt.subplot(n, 5, (i - 1) * 5 + 2)
                plt.imshow(recon_x_pl[i - 1].detach().cpu().numpy(), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pl")

                plt.subplot(n, 5, (i - 1) * 5 + 3)
                plt.imshow(lst_x_pu[0][i - 1].detach().cpu().numpy(), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu1")

                plt.subplot(n, 5, (i - 1) * 5 + 4)
                plt.imshow(lst_x_pu[1][i - 1].detach().cpu().numpy(), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu2")

                plt.subplot(n, 5, (i - 1) * 5 + 5)
                plt.imshow(lst_x_pu[2][i - 1].detach().cpu().numpy(), vmin=0, vmax=1, cmap="gray")
                plt.title("fake_pu3")
    ...
    def accuracy(self, dataset, use_vae=False):
        ...
        for (x_val, y_val) in dataset:
            ...
            tp = tp + torch.sum(torch.greater(c[y_val == 1], 0).int()).detach().cpu().numpy()
            fn = fn + torch.sum(torch.less_equal(c[y_val == 1], 0).int()).detach().cpu().numpy()
            fp = fp + torch.sum(torch.greater(c[y_val == -1], 0).int()).detach().cpu().numpy()
            tn = tn + torch.sum(torch.less_equal(c[y_val == -1], 0).int()).detach().cpu().numpy()
