...
class LBE(nn.Module):
    ...
    def set_classifier_params(self, weights, intercept):
        ...
        state_dict['h.0.weight'] = torch.tensor(weights, dtype=state_dict['h.0.weight'].dtype)
        state_dict['h.0.bias'] = torch.tensor(intercept, dtype=state_dict['h.0.bias'].dtype)
...
class LBE_alternative(nn.Module):
    ...
    def __init_param__(self):
        ...
        return nn.Parameter(torch.cat([weight.reshape(self.input_dim, 1), bias.reshape(1, 1)]), requires_grad=True)
...
class EarlyStopping():
    ...
    def is_better(self, metric_value):
        return (self.best_metric_type == 'max' and metric_value > self.best_metric_value) \
...
def convert_to_DL(samples):
    ...
    x, y, s = samples
    y = np.where(y == 1, 1, 0)
    x, y, s = torch.from_numpy(x).float().to(device), torch.from_numpy(y).float().to(device), torch.from_numpy(s).float().to(device)
...
def train_LBE(train_samples, val_samples, kind='LF', 
        pretraining_epochs=100, pretraining_lr=1e-2, 
        training_max_epochs=100, training_m_step_max_epochs=100, 
        training_lr=1e-3, iter_early_stopping_max_epochs=2,
        M_step_early_stopping_max_epochs=100,        
        verbose=False, log_prefix=''):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ...
    lbe_early_stopping = EarlyStopping(iter_early_stopping_max_epochs, best_metric_type='min')
    for epoch in range(training_max_epochs):
        epoch_early_stopping = EarlyStopping(M_step_early_stopping_max_epochs, best_metric_type='min')
        P_y_hats = []
            ...
            with torch.no_grad():
                def get_metrics(DL):
                    ...
                    for x, y, s in DL:
                        ...
                        y_preds.append(torch.where(lbe(x) > 0.5, 1, 0).reshape(-1).detach().cpu().numpy())
                        eta_preds.append(torch.where(lbe.predict_eta(x) > 0.5, 1, 0).reshape(-1).detach().cpu().numpy())
        ...
        if verbose:
            print(f'{log_prefix}Epoch {epoch}: loss {train_loss:.4f}, acc {train_acc*100:.2f}, val_loss {val_loss:.4f}, val_acc {val_acc*100:.2f}')
...
def eval_LBE(lbe, test_samples, verbose=False, log_prefix=''):
