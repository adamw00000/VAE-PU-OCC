...
def ecdf(X):
    ...
    -------
    ecdf(X) : float
        Empirical CDF function for given dataset
    """
    # introduce non-zero min value to prevent 0 values in log
    X = np.insert(X, 0, -np.inf, axis=0)

    ecdf = ECDF(X)
    return ecdf


def _train_parallel_ecdf(n_dims, X):
    ...
    U_r_mat : numpy array
        ECDF subarray.
    """
    ecdfs_l = np.apply_along_axis(ecdf, axis=0, arr=X)
    ecdfs_r = np.apply_along_axis(ecdf, axis=0, arr=-X)

    return ecdfs_l, ecdfs_r

def _parallel_ecdf(n_dims, X, ecdfs_l, ecdfs_r):
    """Private method to calculate ecdf in parallel.
    Parameters
    ----------
    n_dims : int
    ...
    for i in range(n_dims):
        U_l_mat[:, i] = ecdfs_l[i](X[:, i])
        U_r_mat[:, i] = ecdfs_r[i](-X[:, i])
    return U_l_mat, U_r_mat

def _calculate_ecdf(ecdfs, X):
    ecdf_vals = np.zeros_like(X)
    for dim, dim_ecdf in enumerate(ecdfs):
        ecdf_vals[:, dim] = dim_ecdf(X[:, dim])
    
    return ecdf_vals

class ECODv2(BaseDetector):
    ...
    def __init__(self, contamination=0.1, n_jobs=1):
        super(ECODv2, self).__init__(contamination=contamination)
        self.n_jobs = n_jobs
    ...
    def fit(self, X, y=None):
        ...
        self.decision_scores_ = self.train_decision_function(X)
        self.X_train = X
        self._process_decision_scores()
        return self

    def train_decision_function(self, X):
        ...
        if self.n_jobs != 1:
            return self._train_decision_function_parallel(X)
        
        self.ecdfs_l = np.apply_along_axis(ecdf, 0, X)
        self.ecdfs_r = np.apply_along_axis(ecdf, 0, -X)

        self.skewness = np.sign(skew(X, axis=0))
    ...
    def decision_function(self, X):
        ...
        if not hasattr(self, 'ecdfs_l') or not hasattr(self, 'ecdfs_r') or not hasattr(self, 'skewness'):
            raise NotFittedError("This ECOD instance is not fitted yet. Call 'fit' with appropriate arguments before using this estimator.")
        
        self.U_l = -1 * np.log(_calculate_ecdf(self.ecdfs_l, X))
        self.U_r = -1 * np.log(_calculate_ecdf(self.ecdfs_r, -X))
    ...
    def _train_decision_function_parallel(self, X):
        """Predict raw anomaly score of X using the fitted detector.
         For consistency, outliers are assigned with larger anomaly scores.
        Parameters
        ----------
        ...
        # recover the results
        self.ecdfs_l = np.zeros((n_features,), dtype=np.object)
        self.ecdfs_r = np.zeros((n_features,), dtype=np.object)

        for i in range(n_jobs):
            self.ecdfs_l[starts[i]:starts[i + 1]] = all_results[i][0]
            self.ecdfs_r[starts[i]:starts[i + 1]] = all_results[i][1]
    ...
    def _decision_function_parallel(self, X):
        ...
        all_results = Parallel(n_jobs=n_jobs, max_nbytes=None,
            ...
            delayed(_parallel_ecdf)(
                n_dims_list[i],
                X[:, starts[i]:starts[i + 1]],
                self.ecdfs_l[starts[i]:starts[i + 1]],
                self.ecdfs_r[starts[i]:starts[i + 1]],
            )
            for i in range(n_jobs))
...
# center = 10
# mean_train = [center, 0]
# mean_test = [center / 2, 0]
# std_train = [1, 1]
# std_test = [center / 4, center / 4]
# X_train = np.random.logistic(mean_train, std_train, (N, 2))
# X_test = np.random.logistic(mean_test, std_test, (5 * N, 2))

# # %%
# import matplotlib.pyplot as plt

# plt.figure(figsize=(8, 6))
# plt.plot(X_test[:, 0], X_test[:, 1], 'r.', ms=2)
# plt.plot(X_train[:, 0], X_train[:, 1], 'b.', ms=2)
...
# occ = PyODWrapper(ECOD(contamination=contamination, n_jobs = -1))

# occ.fit(X_train)

# # %%
# plt.figure(figsize=(8, 6))
# plt.scatter(X_test[:, 0], X_test[:, 1], c=occ.score_samples(X_test), s=2)
# # plt.plot(X_train[:, 0], X_train[:, 1], 'b.', ms=2)

# # %%
# n_grid = 100
# x = np.linspace(center - 10, center + 10, n_grid)
# y = np.linspace(-4, 4, n_grid)
# # z = np.array([i*i+j*j for j in y for i in x])

# X, Y = np.meshgrid(x, y)
# z = occ.score_samples(np.concatenate([X.reshape(-1, 1), Y.reshape(-1, 1)], axis=1))
