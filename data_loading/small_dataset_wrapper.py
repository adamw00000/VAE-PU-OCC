...
def gen_trimmed_logit_dataset(N, b, n_features=3, include_bias: bool = False, limit=0.1):
    df = pd.DataFrame()
    for i in range(n_features):
        df[f'X{i+1}'] = np.random.normal(0, 1, N)
...
def create_s_sar_scenario_3(x, y, k=5, p_minus=0.2, p_plus=0.6):
    k = x.shape[1]
    propensity_elems = p_minus + (x - np.min(x, axis=0)) / (np.max(x, axis=0) - np.min(x, axis=0)) * (p_plus - p_minus)
    ...
    new_unlabeled_samples = positives[np.random.random(len(positives)) < 1 - propensity[positives]]
...
def get_small_dataset(name, device, scar, label_frequency, val_size=0.15, test_size=0.15):
    ...
    x_train, x_rest, y_train, y_rest, o_train, o_rest = \
        preprocess(X, y, s, test_size=val_size + test_size, n_best_features=None)
    x_val, x_test, y_val, y_test, o_val, o_test = \
        train_test_split(x_rest, y_rest, o_rest, test_size=test_size / (val_size + test_size))
